...
func main() {
	...
	bestAudioStream := assert(inputCtx.GetBestStream(gmf.AVMEDIA_TYPE_AUDIO)).(*gmf.Stream)
	typ := assert(outputCtx.GuessEncodeCodecId(bestAudioStream.Type())).(int)
	_, outAudioStream := addStream(typ, outputCtx, bestAudioStream)

	videoFg, err := gmf.NewVideoGraph("", []*gmf.Stream{bestVideoStream}, []*gmf.Stream{outVideoStream}, nil)
	defer videoFg.Release()

	audioFg, err := gmf.NewAudioGraph("", []*gmf.Stream{bestAudioStream}, []*gmf.Stream{outAudioStream}, nil)
	defer audioFg.Release()
